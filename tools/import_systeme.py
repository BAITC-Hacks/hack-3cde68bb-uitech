"""Upload the six Systeme XLSX files from the supplied archive. Python >= 3.11."""
import argparse
import json
...
PREFIX_ROLES = {"MOQ": "moq", "Динамика": "sales_transactions", "Ежемесячные остатки": "stock_monthly",
                "Ежемесячные продажи": "sales_monthly", "Сезонность": "seasonality", "Товар в пути": "inventory_transit"}
...
def main():
    ...
    manifest = {"name": "Systeme Electric 22.09.2026", "data_kind": "real", "timezone": "Asia/Almaty",
                "files": [{"part_name": role, "supplier_id": "SYSTEME", "role": role} for role in sorted(files)]}
    ...
    output = Path(__file__).resolve().parents[1] / ".local" / "systeme-import-summary.json"
